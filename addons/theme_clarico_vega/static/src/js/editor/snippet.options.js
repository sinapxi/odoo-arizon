    ...
    options.registry.hotspotSetting = options.Class.extend({
        ...
        async setImgHotspot(previewMode, widgetValue, params) {
            if (widgetValue == 'enabled') {
                ...
                this.$target.parent().append('<section contenteditable="false" name="Hotspot-Block" data-exclude=".s_col_no_resize, .s_col_no_bgcolor" class="hotspot_draggable hotspot_element o_not_editable s_col_no_resize s_col_no_bgcolor fade_hotspot circle"><span class="hotspot_dot fa fa-shopping-bag o_editable_media" draggable="true" contenteditable="true"> </span></section>')
...
//                if (parentEl.tagName !== 'A') {
                    const wrapperEl = document.createElement('a');
                    this.$target.find('.hotspot_dot')[0].after(wrapperEl);
                    wrapperEl.setAttribute('class', 'btn border-0');
                    wrapperEl.appendChild(this.$target.find('.hotspot_dot')[0]);
//                }
//                else{
//                    const wrapperEl = document.createElement('a');
