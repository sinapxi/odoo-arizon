...
class PwaMain(http.Controller):
    ...
    def get_app_image_vals(self, website):
        ...
        for x, y in size_arr.items():
            vals.update({x: "/web/image/website/%s/app_image_512/%s" % (website.id, y) if website.app_image_512 else
            'emipro_theme_base/static/src/img/%s.png' % (y)})
        return vals
    ...
    def service_worker(self):
        ...
        for url in urls:
            version_list.append(url.split('/')[4])
    ...
    @http.route('/pwa_ept/manifest/<int:website_id>', type='http', auth="public", website=True)
